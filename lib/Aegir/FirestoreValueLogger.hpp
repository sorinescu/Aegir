#ifndef __FIRESTORE_VALUE_LOGGER_HPP__
#define __FIRESTORE_VALUE_LOGGER_HPP__

#include "AppendableLogger.hpp"

#define FIRESTORE_VALUE_LOGGER_BLOCK_SIZE 512

template <typename ValueType>
class FirestoreValueLogger: public AppendableLogger<ValueType>
{
    char *_path;
    bool _read_mode;
...
public:
    FirestoreValueLogger() : _file(nullptr), _path(nullptr), _read_mode(false)
    {
    }

    ~FirestoreValueLogger()
    {
        _file.close();
        if (_path)
            free(_path);
    ...
    {
        // ensureReadMode();
        Serial.printf("Firestore '%s' has size %d\n", _file.name(), _file.size());
    ...
    void append(ValueType val) override
    {
        ...
        if (_file.position() % FIRESTORE_VALUE_LOGGER_BLOCK_SIZE == 0)
        {
            Serial.printf("Flushing %d bytes to LittleFS; file size %d bytes\n", FIRESTORE_VALUE_LOGGER_BLOCK_SIZE, _file.size());
...
#endif // __FIRESTORE_VALUE_LOGGER_HPP__
